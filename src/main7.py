...
"""
No tuning -> train and testing is easier to evaluate and no need for 
"""
...
def main_loop(experiment_dir,
              dataset, encoder, scaler, cat_imputer, num_imputer, model, scorings=tuple(), index=0, num_exp=0,
    ...
    # -- Specify solver for LogisticRegression inspired from https://scikit-learn.org/stable/modules/linear_model.html#logistic-regression
    if isinstance(model, u.LogisticRegression):
        model.set_params(**{"solver": "lbfgs" if len(X) < 10000 else "saga"})
    ...
    CT = ColumnTransformer([("encoder", catpipe, cats), ("scaler", numpipe, nums)], remainder="passthrough")
    pipe = Pipeline([("preproc", CT), ("model", model)])
    ...
    print(
        ...
        f"{exec_log['dataset']:40}{exec_log['encoder']:15}{exec_log['model']:10}"
    )
            ...
            for tr, te in cv.split(X, y):
                ...
                for scoring in scorings:
                    out = {
                        ...
                        "cv_score": scoring(yte, pipe.predict(Xte)),
...
models = [
    ...
    u.LogisticRegression(max_iter=10000, random_state=random_state+6, solver="DEPENDS ON DATASET")  # https://scikit-learn.org/stable/modules/linear_model.html#logistic-regression
]
...
if __name__ == "__main__":
    ...
    if test:
        ...
        tempdatasets = ["amazon_employee_access", "adult", "airlines"]
        dids = [u.DATASETS[x] for x in tempdatasets]
        encoders = [e.Discretized(e.TargetEncoder()), e.TargetEncoder()]
        models = [u.LogisticRegression(), u.DecisionTreeClassifier()]
    ...
    experiments = itertools.product(datasets, encoders, scalers, cat_imputers, num_imputers, models)
    ...
    while len(experiments) == 0:
        try:
            ...
            Parallel(n_jobs=nj, verbose=0)(
                delayed(main_loop)(result_folder, dataset, encoder, scaler, cat_imputer, num_imputer, model,
                                   scorings=scorings,
                                   index=index, num_exp=len(experiments), **kwargs)
                for (index, (dataset, encoder, scaler, cat_imputer, num_imputer, model)) in enumerate(experiments)
            )
